...
function checkArithmeticProgression(mass:array of integer;n:integer):boolean;
  var
    i,d:integer;
  begin
    d:= mass[1]-mass[0];
...
procedure writeArr(var arr:array of integer;i,size:integer);
begin
    for i:=0 to size-1 do
      write(arr[i],' ');
end;

  var arrForTest,arrSumOfProgression:array of integer;
      matrixInit, matrixResult:array of array of integer;
      ctd,m,n,i,j,sumOfP:integer;
begin
  ...
  for i:=0 to ctd-1 do
    begin
      for j:=0 to m-1 do
        begin
          write(matrixResult[i][j],' ');
        end;
      writeln(' sum:',arrSumOfProgression[i]);
    end;
